...
public class RestUtility {
    ...
    public void getMeaning(final Activity activity, String reequest, final WebResponseListener listener) {
        ...
        JsonObjectRequest request1 = new JsonObjectRequest(Request.Method.GET, request, null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d("API CALL1", response.toString());
                        RootObject response1 = new Gson().fromJson(response.toString(), RootObject.class);
                        if (listener != null) {
